...
int find_bottom_most_point(vector<pair<float, float>> points) {
    ...
    n = points.size();
    for (i = 1; i < n; i++) {
        if (points[i].second < points[bottom_most].second) bottom_most = i;
        if ((points[i].second == points[bottom_most].second) && (points[i].first < points[bottom_most].first)) bottom_most = i;
    }
    return bottom_most;
}
...
int find_orientation(pair<float, float> p0, pair<float, float> p1, pair<float, float> p2) {
    float cross_product;
    cross_product = (p1.first - p0.first) * (p2.second - p0.second) - (p1.second - p0.second) * (p2.first - p0.first);
...
int compare(const pair<float, float> p1, const pair<float, float> p2) {
    ...
    if (orientation == 0) return distance(bottom_left_most, p2) >= distance(bottom_left_most, p1);
