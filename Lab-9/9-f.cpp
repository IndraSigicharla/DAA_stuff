...
bool node::operator<(const node &a) const { return distance > a.distance; }

bool node::operator==(const node &a) const { return name == a.name; }

void print_graph(const map<string, list<node>> &graph) {
    for (const auto& vertex : graph) {
        cout << vertex.first << endl;
        for (const auto& node : vertex.second) cout << node.name << " " << node.distance << " ";
        cout << endl;
    }
}

int is_adjacent(string vertex, list<node> adj_nodes) {
    for (const auto& node : adj_nodes) if (vertex == node.name) return node.distance;
    return 0;
}
...
void first_step(map<string, list<node>> graph, map<string, table_details> &dij_table, vector<string> vertices, priority_queue<node> &Q, string start) {
    ...
    for (auto v : vertices) {
        ...
        } else new_node.distance = INT_MAX;
        Q.push(new_node);
    }
}
...
void repeated_steps(map<string, list<node>> graph, map<string, table_details> &dij_table, vector<string> vertices, priority_queue<node> &Q, string start) {
    ...
    while (!Q.empty()) {
        min_node = Q.top();
        Q.pop();
        if (dij_table[min_node.name].visited == true) continue;
